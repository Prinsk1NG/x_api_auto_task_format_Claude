...
<REPORT>
  ...
  <PULSE>必须为1句话、60-90字中文，只写总方向，不得塞入多个并列细节主题。</PULSE>
  <THEMES>
...
def llm_call_xai(combined_jsonl: str, today_str: str, memory_context: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            chat.append(system("You are a professional analytical bot. You strictly output in XML format as instructed. Never let PULSE replace THEMES."))
...
def strip_urls(text: str) -> str:
    return re.sub(r"https?://\S+|t\.co/\S+", " ", text or "", flags=re.I).strip()
...
def is_link_only_text(text: str) -> bool:
    ...
    if not t:
        return True
    no_url = strip_urls(t)
    no_punct = re.sub(r"[\W_]+", "", no_url, flags=re.UNICODE)
    return bool(extract_urls(t)) and len(no_punct) == 0


def is_emoji_or_reaction_text(text: str) -> bool:
    t = norm_text(strip_urls(text))
    if not t:
        return True
    ...
    low = t.lower()
    reaction_phrases = {
        "wow", "nice", "great", "amazing", "cool", "true", "based", "exactly",
        "as it should", "troubling", "hmm", "lol", "lmao", "thanks", "thank you",
        "bro", "yes", "no", "yep", "nope", "interesting", "indeed", "try grok",
        "tesla", "grok imagine"
    }
...
def low_information_penalty(text: str) -> float:
    t = norm_text(text)
    if not t:
        return 999999.0
    penalty = 0.0
    ...
    if len(core) < HARD_MIN_MAIN_TWEET_LEN:
        penalty += 900.0
    elif len(core) < MIN_MAIN_TWEET_LEN:
        penalty += 260.0
    if is_link_only_text(t):
        penalty += 1200.0
    elif extract_urls(t) and len(core) < 24:
        penalty += 380.0
    if is_emoji_or_reaction_text(t):
        penalty += 700.0
    if len(words) <= 3:
        penalty += 180.0
    if t.startswith('@') and len(words) <= 8:
        penalty += 220.0
    return penalty


def is_low_information_main_tweet(text: str) -> bool:
    t = norm_text(text)
    if not t:
        return True
    if is_link_only_text(t):
        return True
    if len(strip_urls(t)) < HARD_MIN_MAIN_TWEET_LEN:
        return True
    if is_emoji_or_reaction_text(t):
        return True
    if t.startswith('@') and len(strip_urls(t).split()) <= 6:
        return True
    return False


def enforce_author_diversity_cap(items: list, pool_limit: int) -> list:
    if not items:
        return []
    author_counts = {}
    result = []
    for item in items:
        author = (item.get('author') or '').lower().replace('@', '')
        if not author:
            continue
...
def validate_report_structure(parsed_data: dict) -> tuple:
    themes = parsed_data.get('themes', []) or []
    top_picks = parsed_data.get('top_picks', []) or []
    pulse = norm_text(parsed_data.get('pulse', ''))
    theme_tweet_count = sum(len(t.get('tweets', []) or []) for t in themes)
    issues = []
    if len(themes) < MIN_THEME_COUNT:
        issues.append(f'theme_count={len(themes)}<{MIN_THEME_COUNT}')
    if theme_tweet_count < MIN_THEME_TWEET_COUNT:
        issues.append(f'theme_tweet_count={theme_tweet_count}<{MIN_THEME_TWEET_COUNT}')
    if len(top_picks) < MIN_TOP_PICKS_COUNT:
        issues.append(f'top_picks_count={len(top_picks)}<{MIN_TOP_PICKS_COUNT}')
    if not pulse:
        issues.append('pulse_empty')
    elif len(pulse) > MAX_PULSE_CHARS:
        issues.append(f'pulse_too_long={len(pulse)}>{MAX_PULSE_CHARS}')
